...
#  include the space occupied by the title and colorbar)
plot_width = 1.5
plot_height = 1.5
colorbar_width = 0.75
title_height = 0.25
...
# below to account for title and colorbar dimensions
...
def eval_squareexp_basis(nx, ny, sigma, x, y):
    '''Evaluates the squared exponential control policy basis on the
       ...
       the basis functions from the sigma parameter sigma'''
...
def eval_doob(x, y, theta, sigma, grad=False):
    '''Evaluates the squared exponential control policy ansatz on the
       ...
       and basis sigma parameter sigma'''

    if (theta.ndim != 2):
        raise ValueError(u'Input ``theta`` must be a 2d array')

    (nx, ny,) = theta.shape
    (dfxydx, dfxydy, fxy,) = eval_squareexp_basis(nx, ny, sigma, x, y)
...
if __name__ == '__main__':
    ...
    # - 'basis_shape' : tuple[int, int] --- number of basis function along x and y axes
    # - 'sigma' : float --- scale parameter for all basis functions
    # - 'theta' : numpy.ndarray[numpy.float64] --- amplitude coefficients of basis functions
    ...
    # control policy parameter vector (amplitude coefficients of basis functions)
    theta = data_dict['theta'].ravel()
    theta.shape = (nx, ny,)
